...
public:
  ...
  ~VulkanExample()
  {
    ...
    vkDestroyPipeline(device, pipelines.hmdWarp, nullptr);
    vkDestroyPipeline(device, pipelines.offscreen, nullptr);

    vkDestroyPipelineLayout(device, pipelineLayouts.hmdWarp, nullptr);
  ...
  void buildCommandBuffers()
  {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
    {
      ...
      vkCmdBindDescriptorSets(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayouts.hmdWarp, 0, 1, &descriptorSet, 0, NULL);

      // Final composition as full screen quad
      hmdDistortion->drawQuad(drawCmdBuffers[i], pipelines.hmdWarp);
  ...
  void setupDescriptorSetLayout()
  {
    ...
    VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pPipelineLayoutCreateInfo, nullptr, &pipelineLayouts.hmdWarp));
  ...
  void preparePipelines()
  {
    ...
    pipelineCreateInfo.layout = pipelineLayouts.hmdWarp;
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipelines.hmdWarp));
  ...
  void prepare()
  {
    ...
    hmdDistortion->generateQuads(vulkanDevice, device);
