...
int32_t Gear::newVertex(std::vector<Vertex> *vBuffer, float x, float y, float z, const glm::vec3& normal)
{
	// TODO: remove color
...
void Gear::newFace(std::vector<uint32_t> *iBuffer, int a, int b, int c)
{
	iBuffer->push_back(a);
	iBuffer->push_back(b);
	iBuffer->push_back(c);
...
void Gear::generate(vks::VulkanDevice *vulkanDevice, GearInfo *gearinfo, VkQueue queue)
{
